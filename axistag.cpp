...
MyEllipse::MyEllipse(QCustomPlot *parentPlot) : QCPItemEllipse(parentPlot),
    xRadius(1.0),
    yRadius(1.0),
    mCenterTracer(new QCPItemTracer(parentPlot))
{
    ...
    setRadius(xRadius,yRadius);
}

void MyEllipse::moveCenter(double x, double y) {
    //mCenterTracer->position->setCoords(x, y);
    topLeft->setCoords(-xRadius+x, -yRadius+y);
    bottomRight->setCoords(xRadius+x, yRadius+y);
}

void MyEllipse::setRadius(const double &x, const double &y)
{
    if(x > 0)
        xRadius = x;
    if(y > 0)
    ...
    topLeft->setCoords(-xRadius, -yRadius);
    bottomRight->setCoords(xRadius, yRadius);
}



void MyEllipse::moveTracerCenter(const double &x, const double &y)
{
